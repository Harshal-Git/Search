 ...
 * -> Auxiliary space: 0(1)
 */
public class LastIndexIterativeAP {
	...
	private static int findLastIndex(int[] data, int searchElement, int start, int end) {
		...
		while(start <= end) {
			...
			if(data[mid] < searchElement) {
				...
				// check that current element is not it's last occurrence
				// and we have not encountered last element of the data yet.
				if(mid != (size-1) && (data[mid] == data[mid+1])) {
					// search right half
					start = (mid+1);  
				} else {
					// otherwise current element is last occurrence
					return mid;
				}
			}
		}
